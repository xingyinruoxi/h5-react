...
export default class  extends Component {
        ...
        HTTP.post('/f/app/jxorder/list', {
            ...
            if (res.data.success === 1) {
                ...
                this.setState({
                    data: body.list,
                    loading: false,
                    initDataLength:body.list.length
                }, function () {
                    this.setState({
                        num: body.total
                    })
    ...
    loadData = () => {
        ...
        if (dataHeight - scrollHeight - h < screenHeight && this.state.isFoot) {
            ...
            let {data} = this.state;
            HTTP.post('/f/app/jxorder/list', params).then((res) => {
    ...
    render() {
        let {data, num, loading, finished, isFoot,initDataLength} = this.state;
        let {userType, sessionId} = this.props.userInfo;
        let {touchStart, touchEnd} = this;
        return (
            <div>
                <TabBar/>
                <NavBar title={`我的订单（${num}）`}>
                    {
                    ...
                    <ul>
                        {
                            (!loading)?( data.length > 0 ? (
                                ...
                                >
                                    <CardMenu {...{data, userType, sessionId}} noPrice={true} url={'/listmenu/detail.html'}/>
                                    <p className={'text-center'} style={{'color': '#aaa', 'paddingBottom': '15px'}}>
                                        {
                                            finished ? <span>我是有底线滴哦~</span> :
                                                initDataLength===this.pageSize ? isFoot ? <span>上拉加载更多</span> : <span>请稍等...</span> :
                                                    null
                                        }
                                    </p>
                                </div>
